...
const Orders = () => {
    ...
    useEffect(() => {
        showData("GET", token, rutes.workorder.url +'?state=2')
            .then((resultinfo) => {
                setInfoOrder(resultinfo.data);
        ...
        <main className="Dashboard ">
            <div className="bg-zinc-100">
                ...
                <div className="pl-24 pr-4 md:pl-20 pt-5 md:pr-8 text-tertiary-200 ">
                    <h2 className="text-gray-500 text-3xl font-light p-1 flex items-center justify-center">Gestion Order</h2>
                    <div className="p-1 flex flex-wrap items-center justify-center gap-8 w-4/5">
                        {InfoOrder.map((data) => {
                            return <CardOrder key={data.id} info={data} className="mb-1 mr-4" />;
                        })}
                    </div>
                    <a  className=' z-50 fixed bg-slate-500 text-slate-50 w-20 h-20 rounded-full flex justify-center items-center right-0 mx-4 bottom-1'>
                        <ShoppingCart />
